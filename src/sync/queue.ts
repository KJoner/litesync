...
export class PendingQueue {
	...
	onChange: ((entries: Record<string, PendingAction>) => void) | null = null;
	...
	add(path: string, action: PendingAction): void {
		this.map.set(path, { action, gen: ++this.genCounter });
		this.fireChange();
	}
	...
	entries(): Array<[string, PendingAction, number]> {
		return [...this.map.entries()].map(([p, e]) => [p, e.action, e.gen]);
	}

	toRecord(): Record<string, PendingAction> {
		const out: Record<string, PendingAction> = {};
		for (const [p, e] of this.map.entries()) out[p] = e.action;
		return out;
	}

	/** 从持久化镜像恢复（插件启动时）。 */
	restore(entries: Record<string, PendingAction>): void {
		for (const [path, action] of Object.entries(entries)) {
			if (!this.map.has(path)) this.map.set(path, { action, gen: ++this.genCounter });
		}
		this.fireChange();
	}
