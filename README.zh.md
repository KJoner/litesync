...
1. 原设备本地生成随机配对密钥，把服务器配置（URL + 一次性注册凭据 + 同步设置）
   AES-256-GCM 加密后上传（服务器只存密文，5 分钟过期、一次性；解密密钥只在链接 `#fragment` 中）
2. 新设备用系统相机扫码 → 落地页 →「在 Obsidian 中打开」→ 确认导入
3. 新设备用注册凭据换取**自己的专属设备凭据**（v0.10：根 Token 绝不下发到新设备，
   丢失设备可在服务器上单独撤销）；凭据自动进 SecretStorage；
   **E2EE 密码永不随配对包传输，需手动输入一次**
4. 自动进入接入向导，选「从远端恢复」即完成

也可在设置页手动「导入配对链接」。命令面板「设备列表」可查看所有已注册设备。
